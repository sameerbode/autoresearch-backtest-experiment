...
Exit (first to trigger wins):
  - Price closes below ATR trailing stop (ATR length=9, factor=2.0) — tighter to cut losers
  - Fast EMA crosses below slow EMA
  - Hard stop loss (last resort)

Key change from v4:
  - REMOVED volume confirmation filter — it was hurting win rate (25%), not helping
  - Tightened ATR factor: 2.5 → 2.0 (cut losers faster, improve win rate)
  - bxt_daily_min tightened to 5 (require slightly positive momentum, not just >0)
  - bxt_weekly_min kept at -10 (maintain trade count)
  - rising_bars kept at 2 for momentum confirmation
  
Rationale: Win rate collapsed to 25% with volume filter. The ATR trailing stop
at 2.5 was giving losers too much room. Tighter stops (2.0) should cut losers 
sooner while winners run far enough to maintain good R:R. Removing the volume 
filter recovers trade quality by not filtering late-entry setups.
"""
...
PARAMS = {
    ...
    # BX-Trender (daily)
    "bxt_daily_min": 5,          # slight positive momentum required
    "bxt_daily_rising_bars": 2,  # daily BXT must be higher than N bars ago

    # ATR trailing stop — tightened to cut losers faster
    "atr_length": 9,
    "atr_factor": 2.0,           # tighter than v4's 2.5 — improves win rate

    # Hard stop loss (last resort only)
    "stop_loss_pct": 0.08,       # tightened slightly from 10%
    "max_hold_bars": 120,
}
...
def get_exit_signal(df, i, entry_price, params):
    ...
    Exit when any of:
      ...
      2. ATR trailing stop hit (factor=2.0 — tighter to cut losers faster)
    ...
    # 2. ATR trailing stop (tighter at factor=2.0 — cuts losers faster)
