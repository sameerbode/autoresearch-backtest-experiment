...
Entry (all must be true):
  ...
  6. Volume on crossover bar > N-bar average volume   ← conviction filter (NEW)
  7. Crossover bar closes in upper half of its range  ← bullish price action (NEW)
...
Key change from v14:
  - Added volume confirmation: crossover bar volume must exceed its N-bar MA
    This filters out weak/low-conviction crossovers that tend to fail
  - Added candle close filter: close must be in upper 50% of bar range
    Ensures the crossover bar itself shows bullish intent
  - Slightly tightened bxt_daily_min: 8 → 10 (minor quality improvement)
  
Rationale: Win rate at 39.3% is the weakest component. With 28 trades we have
enough volume that improving quality matters more than quantity. Volume-confirmed
breakouts have historically much higher follow-through rates. The close-in-range
filter eliminates bearish-reversal bars that happen to trigger an EMA cross.
These two lightweight filters should push win rate toward 50%+ while keeping
trade count above 15.
"""
...
PARAMS = {
    ...
    # BX-Trender (daily)
    "bxt_daily_min": 10,          # slightly tightened from 8
    ...
    "volume_min_ratio": 1.0,      # crossover bar volume must be >= this × vol MA

    # Candle close position filter (NEW)
    # Close must be in top X% of the bar's high-low range
    "close_range_min_pct": 0.45,  # close must be above bottom 45% of range (upper 55%)
...
def get_entry_signal(df, i, params):
    ...
    Enter long when ALL conditions are met:
      ...
      6. Volume on crossover bar >= volume_min_ratio × N-bar volume MA  (NEW)
      7. Close is in the upper portion of today's high-low range         (NEW)
    """
    ...
    # This filters out low-conviction crosses that tend to fail
    ...
    # Ensures the crossover bar itself shows bullish conviction (not a wick-heavy reversal)
