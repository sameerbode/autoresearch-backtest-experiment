"""
strategy.py — EMA Cross + Weekly BX-Trender + Daily BX-Trender (rising) + ATR Trailing Stop

Entry (all must be true):
  1. Daily fast EMA freshly crosses above slow EMA    ← entry trigger
  2. Weekly BX-Trender > bxt_weekly_min               ← HTF not in downtrend
  3. Daily BX-Trender > bxt_daily_min AND rising      ← daily momentum confirmed & accelerating
...
Key change from v2:
  - bxt_daily_min raised to 5 (was 0) — require more conviction before entry
  - Daily BX-Trender must ALSO be rising vs 2 bars ago (momentum accelerating)
  - atr_factor reduced to 2.5 (was 2.9) — cut losers faster, improve win rate
  - bxt_weekly_min tightened to -5 (was -10) — require better weekly context
"""
...
PARAMS = {
    ...
    # BX-Trender (weekly) — tightened from -10 to -5
    "bxt_l1": 5,
    "bxt_l2": 20,
    "bxt_l3": 5,
    "bxt_weekly_min": -5,

    # BX-Trender (daily) — raised threshold, requires rising momentum
    "bxt_daily_min": 5,          # must be above 5 (was 0)
    "bxt_daily_rising_bars": 2,  # daily BXT must be higher than N bars ago

    # ATR trailing stop — tighter to cut losers faster
    "atr_length": 9,
    "atr_factor": 2.5,           # reduced from 2.9
...
def get_entry_signal(df, i, params):
    ...
    Enter long when ALL conditions are met:
      ...
      3. Daily BX-Trender > bxt_daily_min (must be above 5, not just positive)
      4. Daily BX-Trender is RISING vs N bars ago (momentum accelerating, not fading)

    The rising check prevents entries when BXT is positive but rolling over —
    those tend to be failed breakouts that hurt win rate.
    """
    ...
    # Condition 3 + 4: Daily BX-Trender — above threshold AND rising
...
def get_exit_signal(df, i, entry_price, params):
    ...
    Exit when any of:
      ...
      2. ATR trailing stop hit (tighter at factor=2.5)
