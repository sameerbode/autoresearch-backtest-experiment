...
Entry (all must be true):
  ...
  2. Weekly BX-Trender > bxt_weekly_min               ← HTF must be positive/acceptable
...
Key change from v10:
  - Tightened weekly BXT threshold: -5 → +5 (require positive weekly momentum)
    The biggest driver of low win rate is entering when the weekly trend is weak/negative.
    With bxt_weekly_min at -5, we allow entries even when weekly momentum is slightly
    bearish. Requiring > +5 ensures we only enter when the weekly trend is genuinely
    supportive — this should meaningfully improve win rate.
  - Slightly tighter daily BXT: 3 → 5, filtering out borderline entries
  - Added EMA slope bars: 1 → 2, requiring momentum to build over 2 bars before cross
    to reduce false breakouts
  - ATR factor: keep at 3.2 — winners need room to run

Rationale: Win rate of 40% is the composite's weakest component and R/R is already
capped at 3.0. Improving win rate from 40% → 50% would lift composite by 25%.
The primary fix is requiring stronger weekly trend context before entering.
"""
...
PARAMS = {
    ...
    "bxt_l2": 20,
    "bxt_l3": 5,
    "bxt_weekly_min": 5,          # was -5 — now require POSITIVE weekly momentum

    # BX-Trender (daily)
    "bxt_daily_min": 5,           # was 3 — slight tightening for quality
    "bxt_daily_rising_bars": 0,   # 0 = disabled

    # Fast EMA pre-slope filter
    "ema_fast_slope_bars": 2,     # was 1 — require 2-bar rising slope before cross

    # Volume confirmation
    "volume_ma_period": 20,
    "volume_ma_mult": 0.8,        # keep loose to avoid rejecting valid setups

    # ATR trailing stop — wider to give trends room
    "atr_length": 9,
    "atr_factor": 3.2,            # keep at 3.2 — winners need room to run
...
def get_entry_signal(df, i, params):
    ...
    Enter long when ALL conditions are met:
      ...
      2. Weekly BX-Trender > bxt_weekly_min (now requires POSITIVE weekly momentum)
      3. Daily BX-Trender > bxt_daily_min (daily momentum confirmed)
    ...
    # Using slope_bars=2 means we need 2 consecutive rising bars before the cross
    ...
    # Condition 2: Weekly BX-Trender must be POSITIVE (> +5)
    # This is the key improvement — only enter when weekly trend is genuinely bullish
    bxt_w = _weekly_bx_trender(df, i, params["bxt_l1"], params["bxt_l2"], params["bxt_l3"])
    if bxt_w is None or bxt_w <= params["bxt_weekly_min"]:
        return False

    # Condition 3: Daily BX-Trender — needs to be above threshold (> 5)
...
def get_exit_signal(df, i, entry_price, params):
    ...
    Exit when any of:
      ...
      2. ATR trailing stop hit (factor=3.2 — wider to give trends room)
    ...
    # 2. ATR trailing stop (factor=3.2 — wider to reduce premature exits)
